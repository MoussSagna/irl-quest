...
jest.mock('../api', () => ({
  apiClient: { me: jest.fn(), signIn: jest.fn(), signUp: jest.fn(), signOut: jest.fn() },
  apiConfiguration: { baseUrl: 'http://test-api', error: null },
}));
...
function Probe() {
  const auth = useAuth();
  return <>{auth.isLoading ? <Text>loading</Text> : <Text>{auth.configurationError?.message ?? (auth.isAuthenticated ? auth.user?.email : 'signed-out')}</Text>}</>;
}
